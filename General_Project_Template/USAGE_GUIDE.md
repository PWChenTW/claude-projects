...
#### 2. 上下文工程準備 (Context Engineering)

##### 2.1 生成實作藍圖
```bash
> /spec-generate-prp user-auth
```
...
生成的文件：`.kiro/specs/user-auth/implementation_blueprint.md`

##### 2.2 深度分析 (ULTRATHINK)
```bash
> /spec-ultrathink user-auth
```
...
生成的文件：`.kiro/specs/user-auth/ultrathink_analysis.md`

##### 2.3 驗證上下文完整性
```bash
> python .claude/scheduler/context_validator.py user-auth
```
