...
class QualityChecker:
    ...
    def check_recent_files(self, minutes=10):
        ...
        # 檢查各種類型的文件
        file_patterns = ['**/*.py', '**/*.js', '**/*.ts', '**/*.go', '**/*.java', 'src/**/*', 'strategies/**/*']
    ...
    def _check_file_content(self, filepath):
        ...
        try:
            ...
            # 量化交易特定檢查
            self._check_trading_logic(filepath, content)
            self._check_risk_controls(filepath, content)
    ...
    def _check_sensitive_data(self, filepath, content):
        ...
        sensitive_patterns = [
            ...
            (r'exchange.?key\s*=\s*["\'][^"\']{10,}', '交易所密鑰可能被硬編碼'),
        ]
    ...
    def _check_trading_logic(self, filepath, content):
        """檢查交易邏輯相關問題"""
        # 檢查是否有數值計算但沒有處理浮點精度
        if re.search(r'price|amount|quantity|volume', content, re.IGNORECASE):
            if not re.search(r'round|decimal|quantize', content, re.IGNORECASE):
                self.warnings.append(
                    f"💰 提醒: {filepath} 涉及價格/數量計算但可能未處理浮點精度"
                )
        
        # 檢查是否有硬編碼的交易參數
        hardcoded_patterns = [
            (r'leverage\s*=\s*\d+', '槓桿可能被硬編碼'),
            (r'position_size\s*=\s*\d+', '倉位大小可能被硬編碼'),
            (r'stop_loss\s*=\s*[\d.]+', '止損點可能被硬編碼'),
            (r'take_profit\s*=\s*[\d.]+', '止盈點可能被硬編碼'),
        ]
        
        for pattern, description in hardcoded_patterns:
            if re.search(pattern, content, re.IGNORECASE):
                self.warnings.append(
                    f"⚙️ 建議: {filepath} {description}，建議使用配置文件"
                )
    
    def _check_risk_controls(self, filepath, content):
        """檢查風險控制相關"""
        # 檢查交易相關函數是否有風控檢查
        trading_patterns = [
            r'place_order',
        ...
        has_trading_functions = any(
            re.search(pattern, content, re.IGNORECASE)
            for pattern in trading_patterns
        )
        
        if has_trading_functions:
            # 檢查是否有風控相關代碼
            risk_patterns = [
                r'check_risk',
                r'risk_limit',
            ...
            if not has_risk_controls:
                self.warnings.append(
                    f"⚠️ 風控提醒: {filepath} 有交易操作但可能缺少風險檢查"
                )
