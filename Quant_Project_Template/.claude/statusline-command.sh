...
# Claude Code Statusline Script for Quant Trading Template
...
# Trading environment indicator (based on directory names or config files)
trading_env=""
if [[ "$current_dir" == *"backtest"* ]] || [[ -f "backtest_config.py" ]]; then
    trading_env="📈 Backtest"
elif [[ "$current_dir" == *"live"* ]] || [[ -f "live_config.py" ]]; then
    trading_env="🔴 Live"
elif [[ "$current_dir" == *"paper"* ]] || [[ -f "paper_config.py" ]]; then
    trading_env="📝 Paper"
elif [[ -f "strategy.py" ]] || [[ -f "strategies/" ]]; then
...
# Trading environment (if detected)
if [[ -n "$trading_env" ]]; then
    statusline+=" $trading_env"
fi
